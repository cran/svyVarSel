...
#'
#'@description This function allows as to fit elastic nets (linear or logistic) to complex survey data, considering sampling weights in the estimation process and selects the lambda that minimizes the error based on different replicate weights methods.
#'
...
#' @param alpha A numeric value between 0 and 1, indicating the elastic-net parameter. \code{alpha=1} is the LASSO penalty (equivalent to using \code{wlasso()}) and \code{alpha=0} the ridge penalty. Default \code{alpha=1}.
...
#'
#' @details
#' The parameter \code{alpha} indicates the penalty to be used to fit the elastic net as in \code{glmnet}:
#'
...
#'
#' @return The output object of the function \code{welnet()} is an object of class \code{welnet}. This object is a list containing 4 or 5 elements, depending on the value set to the argument \code{print.rw}. Below we describe the contents of these elements:
...
welnet <- function(data = NULL, col.y = NULL, col.x = NULL,
  ...
  if(alpha < 0 | alpha > 1){stop("Please, set a value between 0 and 1 for the argument 'alpha'.")}
